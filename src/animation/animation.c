...
/*   Updated: 2025/03/03 14:32:20 by anpicard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_animation(t_game *data, int frame_count, int frame_delay,
		char **frames)
{
	int	i;
	...
	while (i < frame_count)
	{
		...
		data->img[i] = mlx_xpm_file_to_image(data->mlx, frames[i], &(int){0},
				&(int){0});
		if (!data->img[i])
		{
